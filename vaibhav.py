...
bot = telebot.TeleBot('7278587045:AAFVT8w5CpMx0Zs1Fj3P8x6mRyfrf60zFAc')
...
def show_all_users(message):
    ...
    if user_id in admin_id:
        ...
        response = "Only @Vkedits_31 Can Run This Command."
    bot.reply_to(message, response)

@bot.message_handler(commands=['owner'])
def show_owner(message):
    response = "👑 Bot Owner: @Vkedits_31"  # Replace with the actual owner username
    bot.reply_to(message, response)
        ...
        if len(command) > 2:  # Check if the command contains the user ID and time duration
            ...
            try:
                ...
                if user_to_add not in allowed_user_ids:
                    ...
                    save_user_access(user_access)
                    response = f"User {user_to_add} approved for {time_value} {time_unit} by @Vkedits_31.\n\n\n 🅑🅞🅣 🅛🅘🅝🅚: @Vk_VVIPbot"
                else:
...
def handle_bgmi(message):
    ...
    if user_id in allowed_user_ids:
        ...
        response = ("🚫 Unauthorized Access! 🚫\n\n"
                    ...
                    "👉 Contact an Admin or the Owner @Vkedits_31 for approval.\n"
                    "🌟 Become a proud supporter and purchase approval.\n"
                    "💬 Chat with an Owner @Vkedits_31 now and level up your capabilities!\n\n"
...
def clear_logs_handler(message):
    ...
    if user_id in admin_id:
        ...
        bot.reply_to(message, "Only @Vkedits_31 can run this command.")
